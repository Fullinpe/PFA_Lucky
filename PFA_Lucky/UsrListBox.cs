    ...
    public class UsrListBox : ListBox
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                g.DrawString(Items[i].IP, font, new SolidBrush(Color.FromArgb(128, 128, 128)), fontLeft,
                    bounds.Top + 20);
                g.DrawString(Items[i].Mac, font, new SolidBrush(Color.FromArgb(128, 128, 128)), fontLeft,
                    bounds.Top + 35);

                if (Items[i].Image != null)
                {
                    g.InterpolationMode = System.Drawing.Drawing2D.InterpolationMode.HighQualityBilinear;
                    g.DrawImage(Items[i].Image,
                        new Rectangle(bounds.X + 5, (bounds.Height - 40) / 2 + bounds.Top, 40, 40));
                }

//TODO:
                g.DrawImage(Items[i].Image,
                    new Rectangle(bounds.Width - 28, (bounds.Height - 16) / 2 + bounds.Top, 16, 16));
            }
    ...
    public class ListBoxItemCollection : IList
    {
        private UsrListBox m_owner;

        public ListBoxItemCollection(UsrListBox owner)
        {
            this.m_owner = owner;
        }

        internal UsrListBox Owner
        {
